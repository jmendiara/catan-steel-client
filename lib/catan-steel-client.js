...
CatanClient.prototype.setup = function setup() {
  this._backend.hear(protocol.TALK.HOLA_DON_PEPITO);
  this._backend.say(protocol.TALK.HOLA_DON_JOSE);
  this._backend.hear(protocol.TALK.PASO_USTED_POR_MI_CASA);
  this._backend.go(protocol.HOMES.DON_JOSE);
  this._backend.say(protocol.TALK.POR_SU_CASA_YO_PASE);
  this._backend.hear(protocol.TALK.VIO_USTED_A_MI_ABUELA);
  this._backend.go(protocol.HOMES.ABUELA);
  this._backend.say(protocol.TALK.A_SU_ABUELA_YO_LA_VI);
  this._backend.hear(protocol.TALK.ADIOS_DON_PEPITO);
  this._backend.say(protocol.TALK.ADIOS_DON_JOSE);
};

/**
 * Chops steel form the backend
 * @param {Function} cb The chop callback.
 */
CatanClient.prototype.chop = function chop(cb) {
  this._backend.chop(cb);
};
